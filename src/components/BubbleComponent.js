...
const BubbleComponent = ({ config, index, className, isFloating = false }) => {
    ...
    const createSplashEffect = () => {
      ...
      const dropletCount = Math.floor(bubbleSize / 3);
  
      for (let i = 0; i < dropletCount; i++) {
        const droplet = document.createElement('div');
        droplet.className = 'droplet';
        
        const dropletSize = bubbleSize;
